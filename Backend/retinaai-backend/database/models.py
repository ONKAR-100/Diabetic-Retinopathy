...
class Patient(Base):
    ...
    simulations = relationship("RetinalSimulation", back_populates="patient", cascade="all, delete-orphan")
...
class RetinalSimulation(Base):
    """
    Retinal computational simulation record generated by the Simulink state-space model.

    DISCLAIMER:
    Outputs represent engineering/computational states over dimensionless relaxation time
    theta in [0, 10]. They are NOT clinical risk, DR grade, or disease progression measures.
    """
    __tablename__ = "retinal_simulations"

    id = Column(String, primary_key=True, default=generate_uuid)
    screening_id = Column(String, ForeignKey("screenings.id", ondelete="CASCADE"), nullable=False, index=True)
    patient_id = Column(String, ForeignKey("patients.id", ondelete="CASCADE"), nullable=False, index=True)
    eye = Column(String, nullable=False)  # 'left' or 'right'

    model_name = Column(String, default="retina_computational_state")
    ...
    normalized_inputs = Column(JSON, nullable=False)
    output_state = Column(JSON, nullable=False)
    trajectory = Column(JSON, nullable=False)

    execution_status = Column(String, nullable=False)  # 'completed', 'failed', 'disabled', 'invalid_input'
    runtime_seconds = Column(Float, nullable=True)
    error_message = Column(Text, nullable=True)
    created_at = Column(DateTime, default=utc_now)

    screening = relationship("Screening", back_populates="simulations")
